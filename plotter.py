...
def plot(inputs, plotting_mode="separate"):
    ...
    for file in files:
        ...
        if (plotting_mode=="separate") or ("numerical" in file):
            ...
            #Saves the graph as the name of the original csv file but as a png instead and also contains the input variables in the name
            plt.savefig(f"{file[:-4]} - {subtitle.replace("=", "_")}.png")
